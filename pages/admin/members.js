...
const error = null
  ...
  // Fetch CE hours aggregated by email and designation for selected year
  ...
  // Build hours lookup: email → { nssa: X, irmaa: Y, lastDate: '...' }
  const hoursMap = {}
  ...
  // Merge members with their CE hours
...
export default function AdminMembers({ rows, selectedYear, availableYears, currentYear }) {
  ...
  const [designFilter, setDesignFilter] = useState('all')   // all | nssa | irmaa
  const [statusFilter, setStatusFilter] = useState('all')   // all | met | progress | unstarted | exempt
  ...
  // KPI stats from full filtered set (before pagination)
  ...
  function exportCSV() {
    ...
    a.download = `nssa-ce-report-${selectedYear}.csv`; a.click()
  }
    ...
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ background: 'white', border: `1px solid ${GRAY.border}`, borderRadius: '8px', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={thStyle('lastName')} onClick={() => handleSort('lastName')}>Name <SortIcon col="lastName" /></th>
          ...
          <tbody>
            ...
            {paginated.length === 0 && (
              <tr><td colSpan={7} style={{ padding: '2.5rem', textAlign: 'center', color: GRAY.text, fontSize: '13px' }}>
